...
def test_empty(tmpdir, write_engine, read_engine, index):
    ...
    if (
        write_engine.startswith("pyarrow")
        and read_engine.startswith("pyarrow")
        and index is False
    ):
        partition_sizes = [(0,), None]
    else:
        partition_sizes = (0,)
    assert_eq(ddf, read_df, partition_sizes=partition_sizes)
...
def test_gather_statistics_no_index(tmpdir, write_engine, read_engine):
    ...
    ddf2 = dd.read_parquet(fn, engine=read_engine, index=False)
    assert_eq(
        ddf,
        ddf2,
        check_index=False,
        divisions=[tuple(range(0, 420, 30)), (None,) * 14],
...
def test_read_dir_nometa(tmpdir, write_engine, read_engine, statistics, remove_common):
    ...
    if statistics is False or (
        statistics is None and read_engine.startswith("pyarrow")
    ):
        partition_sizes = [(3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 4), None]
    else:
        partition_sizes = True
    assert_eq(
        ddf,
        ddf2,
        check_divisions=False,
...
def test_chunksize(tmpdir, chunksize, engine, metadata):
    ...
    assert_eq(ddf1, ddf2, check_divisions=False, partition_sizes=[(50, 50), False])
...
def test_multi_partition_none_index_false(tmpdir):
    ...
    ddf2 = dd.read_parquet(str(tmpdir), index=False, gather_statistics=True)
    assert_eq(
        ddf1,
        ddf2,
        check_index=False,
...
def test_pyarrow_dataset_partitioned(tmpdir, engine, test_filter):
    ...
    if test_filter:
        ...
        assert_eq(ddf, read_df, partition_sizes=[(3,), (1, 2)])
...
def test_create_metadata_file(tmpdir, write_engine, read_engine, partition_on):
    ...
    if partition_on:
        ...
        pd.testing.assert_frame_equal(df1.sort_values("b"), ddf2)
    else:
        assert_eq(ddf1, ddf2)
