...
def test_categories_unnamed_index(tmpdir, engine):
    ...
    assert_eq(
        ddf.index, ddf2.index, check_divisions=False, partition_sizes=[None, (3,)]
    )
...
def test_pyarrow_schema_inference(tmpdir, index, engine, schema):
    ...
    # `set_index` above loses partition_size info
    ...
    if index and engine == "fastparquet":
        ...
        assert_eq(
            ...
            partition_sizes=check_partition_sizes,
        )
    else:
        assert_eq(df, df_out, partition_sizes=check_partition_sizes)
...
def test_read_glob_no_meta(tmpdir, write_engine, read_engine):
    ...
    assert_eq(ddf, ddf2, check_divisions=False, partition_sizes=False)
...
def test_chunksize(tmpdir, chunksize, engine, metadata):
    ...
    assert_eq(ddf1, ddf2, check_divisions=False, partition_sizes=False)
