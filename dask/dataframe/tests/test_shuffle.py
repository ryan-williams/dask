...
def test_set_index_nan_partition():
    ...
    assert_equal(a_gt3.compute().index.values, list(range(4, 10)))

    a_gt1 = d[d.a > 1].set_index("a", sorted=True)  # Set sorted index with 0 null partitions
    assert a_gt1.divisions == (2, 4, 7, 9)
    assert a_gt1.partition_sizes == (2, 3, 3)
    assert_equal(a_gt1.compute().index.values, list(range(2, 10)))

    a_gt3_sorted = d[d.a > 3].set_index("a", sorted=True)  # Set sorted index with 1 null partition
    assert a_gt3_sorted.divisions == (4, 4, 7, 9)
    assert_eq(a_gt3_sorted, a_gt3, partition_sizes=[(0, 3, 3), None])
