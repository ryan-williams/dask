...
Dask Name: {name}, {task} tasks"""
    ...
    def std(
        ...
        if axis == 1:
            ...
            result = map_partitions(
                np.sqrt, v, meta=meta, token=name, enforce_metadata=False, preserve_partition_sizes=True
            )
            return handle_out(out, result)
...
def quantile(df, q, method="default"):
    ...
    return return_type(graph, name2, meta, new_divisions, partition_sizes=new_partition_sizes)
...
def repartition_sizes(df, sizes):
    ...
    result.partition_sizes = tuple(sizes)  # TODO: push this into the concat machinery (TODO: I think that was done?)
    return result
